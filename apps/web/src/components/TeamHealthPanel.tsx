...
import { TEAM_TIER_LABELS, TEAM_TIER_SORT_ORDER } from "../constants";
import { buildDepartmentGroups } from "../utils/org";
import { TeamFlagsAiStatus } from "./TeamFlagsAiStatus";
import { TeamFlagSkeletonCard } from "./TeamFlagSkeletonCard";
...
export function TeamHealthPanel({
  ...
  const showTeamGrid = !error && hasTeams && (isLoading || (teams?.length ?? 0) > 0);
  ...
  return (
    <section className="team-flags">
      ...
      {hasTeams && !isLoading && !error && !teams?.length ? (
        <p className="summary-placeholder">
          Click <strong>Generate team insights</strong> to produce green flags, red
          flags, and a suggested action for each team in <strong>{company}</strong>.
        </p>
      ) : null}
    </section>
  );
