...
import { mapTeamDisplayName, normalizeStatus } from "./org";
...
export function parseLastConnectedAt(rawValue: string): Date | null {
...
export function alignRowValues(headers: string[], values: string[]): string[] {
...
export function deriveStatus(rawRecord: RawRecord, lastConnectedAt: Date | null): Status {
...
export function parseCsv(text: string): Person[] {
  ...
  return lines.slice(1).map((line, rowIndex) => {
    ...
    const rawRecord = headers.reduce<RawRecord>((record, header, columnIndex) => {
